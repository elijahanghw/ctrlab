...
dronelib.vec_integrate_rk4.argtypes = [
...
dronelib.vec_integrate_euler.argtypes = [
    ctypes.c_int,                    # num_envs
    ctypes.c_float,                  # mass
    ctypes.POINTER(ctypes.c_float),  # I
    ctypes.POINTER(ctypes.c_float),  # I_inv
...
class Drone3D_vec:
    ...
    def step(self, U, ground_collision=False):
        ...
        # Call the C function
        if self.integration == 'rk4':
            dronelib.vec_integrate_rk4(self.num_envs, self.mass, c_I, c_I_inv, c_G1, c_states, c_U, self.dt)
            # self.states[3:7] = quat_normalize(self.states[3:7])
            
        elif self.integration == 'forward_euler':
            dronelib.vec_integrate_euler(self.num_envs, self.mass, c_I, c_I_inv, c_G1, c_states, c_U, self.dt)
            # self.states[3:7] = quat_normalize(self.states[3:7])
